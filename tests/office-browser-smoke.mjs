...
// Issue #5: a course that meets several days a week is one record with several
// selected weekdays, and it must appear on each of them.
const weekdayExercise = `<script>setTimeout(async()=>{if(isMobile())return;try{state.semester={name:'Test',startDate:'2026-09-07',weeks:16};state.courses=[];state.exceptions=[];selectedWeek=1;activeView='timetable';await render();$('add').click();$('course-name').value='Multi day course';[...$('weekday').options].forEach((option)=>{option.selected=['1','3'].includes(option.value)});$('start-time').value='08:00';$('end-time').value='09:40';$('start-week').value='1';$('end-week').value='16';$('course-form').requestSubmit();await new Promise((done)=>setTimeout(done,50));const saved=state.courses.find((course)=>course.name==='Multi day course');if(!saved)throw Error('the course dialog did not save');if(String(courseWeekdays(saved))!=='1,3')throw Error('saved weekdays: '+courseWeekdays(saved));const days=[...document.querySelectorAll('#timetable .day')].filter((day)=>day.querySelector('.class-card')).map((day)=>day.querySelector('h3').textContent.trim());if(days.length!==2)throw Error('timetable day sections with classes: '+days.join('/'));if(!days.some((day)=>day.includes('Mon'))||!days.some((day)=>day.includes('Wed')))throw Error('timetable missed a weekday: '+days.join('/'));const detail=document.querySelector('#courses .course-description .muted').textContent;if(!detail.includes('Mon')||!detail.includes('Wed'))throw Error('course list details: '+detail);document.body.insertAdjacentHTML('beforeend','<pre id="weekday-smoke">PASS multi-weekday course dialog, timetable and list</pre>')}catch(error){document.body.insertAdjacentHTML('beforeend','<pre id="weekday-smoke">FAIL '+error.stack+'</pre>')}},800)</script>`;
...
const page = source.replace('<script>', `${stub}<script>`).replace('</body>', `${packaged ? packagedExercise : exercise + mobileExercise + courseListExercise + mergedJumpExercise + weekdayExercise}</body>`);
...
try {
  ...
  if (packaged) expectMarker(stdout, 'package-smoke', 'packaged plugin');
    ...
    expectMarker(stdout, 'weekday-smoke', 'multi-weekday course');
  }
