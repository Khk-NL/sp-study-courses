...
try {
  ...
  if (packaged) assert.match(stdout, /PASS packaged plugin renders/);
  else {
    assert.match(stdout, /PASS XLSX DOCX HTML merged cells multi-sheet/);
    assert.match(stdout, /PASS course list layout sorting jump/);
    assert.match(stdout, /PASS merged course jump/);
  }
  ...
  if (!packaged) assert.match(mobile.stdout, /PASS mobile Today and single-day timetable/);
  else assert.match(mobile.stdout, /PASS packaged plugin renders/);
  console.log(packaged ? 'packaged plugin desktop/mobile smoke passed' : 'browser Office, HTML and mobile smoke passed');
} finally {
  server.close();
